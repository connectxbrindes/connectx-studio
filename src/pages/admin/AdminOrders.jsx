...
export default function AdminOrders() {
  ...
  const exportReport = () => {
    ...
    const data = repRows.map((o) => ({
      ...
      Unidade: o.reseller?.short_name || o.reseller?.contact_name || o.reseller_name || '',
  ...
  const columns = [
    ...
    { key: 'customer_name', label: 'Cliente', render: (o) => o.customer_name },
    {
      key: 'reseller',
      label: 'Unidade',
      // Nome curto da unidade: prioriza o Apelido (short_name); senão usa o
      // "Nome do Contato" (contact_name), onde já vivem as abreviações
      // (ex: KingCase JK); por fim, o nome completo. Completo fica no tooltip.
      render: (o) => (
        <span className="font-medium text-text-primary" title={o.reseller?.name || o.reseller_name || ''}>
          {o.reseller?.short_name || o.reseller?.contact_name || o.reseller_name || '—'}
        </span>
      ),
    },
    {
